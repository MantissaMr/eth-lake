...
/// 
async fn process_block(rpc_url: String, block_number: u64, output_dir: String) -> anyhow::Result<String> {
    // -- SETUP CONNECTION --
    ...
    number_builder.append_value(block_data.header.number
        .unwrap_or_default());
    hash_builder.append_value(block_data.header.hash
        .map(|h| h
        .to_string())
        .unwrap_or_default());
    parent_builder.append_value(block_data.header.parent_hash
        .to_string());
...
fn fetch_block_arrow(py: Python<'_>, rpc_url: String, block_number: u64, output_path: String) -> PyResult<&PyAny> {
    future_into_py(py, async move {
        ...
        .await
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Error processing block: {}", e)))
    })
}

#[pyfunction]
fn extract_range(py: Python<'_>, rpc_url: String, start: u64, end: u64, output_dir: String, concurrency: usize) -> PyResult<&PyAny> {
    future_into_py(py, async move {
        let stream = stream::iter(start..=end)
            .map(|block_number| {
                ...
                async move {
                    process_block(url, block_number, dir).await
                }
            })
